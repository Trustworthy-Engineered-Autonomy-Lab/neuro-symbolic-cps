...
def Robust_MPC_two_error(x0, error_bound, penalty_start, N):
    ...
    pos1 = x0[0]; pos2 = x0[1]; pos3 = x0[2]
    pos_lb = error_bound[0]; pos_ub = error_bound[1]

    opti = ca.Opti()

    # Variables
    x = opti.variable(1, N + 1)  # State trajectory (position and velocity)
...
success_result = set(); fail_result = set(); errors = set()
j = int(input('Enter Number Of Iterations: '))
for i in range(1, j + 1):
    ...
    lower_bound = (0.02961 / j) * i
    upper_bound = (0.02180 / j) * i
    perception_error_bound = (-lower_bound, upper_bound)
    print(f'Iteration: {i}, Current Error Bound: {perception_error_bound}')
    x = input("Press enter to continue")
    errors.add(perception_error_bound)
    x1_hat = x1 + np.random.uniform(perception_error_bound[0], perception_error_bound[1])
    x2_hat = x2 + np.random.uniform(perception_error_bound[0], perception_error_bound[1])
    x3_hat = x3 + np.random.uniform(perception_error_bound[0], perception_error_bound[1])

    ## 3. put the percepts into the MPC
    x0 = np.array([x1_hat, x2_hat, x3_hat])
    error_bound = np.array([lower_bound, upper_bound])
    ...
    for k in range(N - 3):
        # Mountain Car dynamics
        force_k = Robust_MPC_two_error(x0, error_bound, penalty_start, N)

        # Apply the Mountain Car dynamics model
        velocity_next = velocity_k + (0.0015 * force_k - 0.0025 * ca.cos(3 * position_k))
        position_next = position_k + velocity_next + np.random.uniform(perception_error_bound[0], perception_error_bound[1])
        ...
        x0 = np.array([x0[1], x0[2], position_k])
    ...
    if any(pos > 0.45 for pos in test_pos_list):
        ...
        plt.plot(test_pos_list, label=f'{i}, {perception_error_bound}')
    else:
...
print(len(success_result) / (len(success_result) + len(fail_result)))
print(errors)
