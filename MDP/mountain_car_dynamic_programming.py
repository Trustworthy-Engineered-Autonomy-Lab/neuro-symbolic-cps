...
if __name__ == '__main__':
    ...
    """
    conformal_bounds = {
       (-1.2, -0.84): (-0.01067, 0.01067),
       (-0.84, -0.48): (-0.00864, 0.00730),
       (-0.48, -0.12): (-0.01968, 0.01006),
       (-0.12, 0.24): (-0.01250, 0.01178),
       (0.24, 0.6): (-0.01090, 0.01093)
    }
    """
    conformal_bounds = [
        (error_multiplier * -0.01067, error_multiplier * 0.01067), 
        (error_multiplier * -0.00864, error_multiplier * 0.00730), 
        (error_multiplier * -0.01968, error_multiplier * 0.01006), 
        (error_multiplier * -0.01250, error_multiplier * 0.01178), 
        (error_multiplier * -0.01090, error_multiplier * 0.01093)
    ]
    ...
    for __ in range(num_episodes):
        ...
        for t in range(T):
            ...
            if true_observation >= -1.2 and true_observation < -0.84:
                observation[0] += np.random.uniform(conformal_bounds[0][0], conformal_bounds[0][1])
            elif true_observation >= -0.84 and true_observation < -0.48:
                observation[0] += np.random.uniform(conformal_bounds[1][0], conformal_bounds[1][1])
            elif true_observation >= -0.48 and true_observation < -0.12:
                observation[0] += np.random.uniform(conformal_bounds[2][0], conformal_bounds[2][1])
            elif true_observation >= -0.12 and true_observation < 0.24:
                observation[0] += np.random.uniform(conformal_bounds[3][0], conformal_bounds[3][1])
            elif true_observation >= 0.24 and true_observation <= 0.6:
                observation[0] += np.random.uniform(conformal_bounds[4][0], conformal_bounds[4][1])
            ...
            positions.append(true_observation)
            observation[0] = true_observation
            observation[1] = true_velocity
